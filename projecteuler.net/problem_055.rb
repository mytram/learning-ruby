...
def lychrel?(n, iter=50, lychrels = {})
  50.times do
    n = n + n.to_s.reverse.to_i
    return lychrels[n] if lychrels.key?(n)
    return lychrels[n] = false if palindrome?(n)
  end
  lychrels[n] = true
end
...
def solve(problem)
  lychrels = {}
  (1..problem).select { |n| lychrel?(n, lychrels) }.size
end

puts "#{__FILE__} #{solve(10_000)}"
